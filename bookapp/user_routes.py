...
def search_book():
    ...
    search_title = "%" + title + "%" # option 2 here %{}%.format(title)
    #run query
    result = db.session.query(Book).filter(Book.book_catid_==cate).filter(Book.book_title.ilike(search_title)).all()
...
def donate():
    ...
    if request.method =='GET':
        ...
        db.session.add(don) 
        db.session.commit()     
        #save the refno in a session so that we can retrieve the detailks on the next page
        session['ref'] = refno  
...
@app.route("/payment")
def make_payment():
    userdeets = db.sesssion.query(User).get(session.get('userid'))
    if session.get('ref') !=None:
        ref = session['ref']
        #TO DO: we want to get the details of the 
...
def paystack():
       if session.get('ref') !=None:
        ...
        # we want to connect to paystack api
            ...
            headers = {"Content-Type": "application/json","Authorization":"Bearer sk_test_4aa4c20b53e02cdeb04a8fe0da970d44aec8860b"}
...
def home():
    ...
    useronline=session.get('userid')
    userdeets=db.session.query(User).get(useronline)
    #we will connect to the endpoint http://127.0.0.1:5000/api
